...
import mocpic from "@/assets/blog.png"
import type { ClinicFormData } from "@/components/modal/ClinicFormModal";
...
const AdminClinic : React.FC = () => {
  const [openModal, setOpenModal] = useState(false);
  const [editData, setEditData] = useState<ClinicFormData | null>(null);
  ...
  const handleEdit = (id: number) => {
    const Clinics = mockData.find((d) => d.id === id);
    if (Clinics) {
      setEditData(Clinics);
      setOpenModal(true);
    }
  ...
  // SUBMIT FORM
  const handleSubmitForm = (data: ClinicFormData) => {
